...
public class FabnSceneLvl2 implements Initializable {
    ...
    @FXML
    private ImageView tir;

    @FXML
    private ImageView tir1;
    ...
    {
        TranslateTransition TIR = new TranslateTransition();
        TIR.setNode(tir);
        TIR.setDuration(Duration.millis(800));
        TIR.setByX(+450);
        TIR.play();
        ...
        if (tir.getBoundsInParent().intersects(pic2.getBoundsInParent())){
        ...
        if (tir1.getBoundsInParent().intersects(pic3.getBoundsInParent())) {
